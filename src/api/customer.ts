...
import type { Customer, CustomerActivity, CustomerNote } from '../types/customer';
...
class CustomerApi {
  async getCustomers(options: Options): Promise<{ customers: Customer[], customersCount: number }> {
    if (throttle) {
      await wait(throttle);
    }
  ...
  async getCustomerOrders(options: Options = {}): Promise<Order[]> {
    if (throttle) {
      await wait(throttle);
    }
  ...
  async getCustomerNotes(): Promise<CustomerNote[]> {
    if (throttle) {
      await wait(throttle);
    }
